#!/usr/bin/env python3
"""MCP server for Next.js official documentation (nextjs.org/docs).

Serves the local mirror at /Users/ahmed/Downloads/Next.js-Docs
that was synced from vercel/next.js canary branch.
"""
import json
import re
import sys
...
def _all_doc_files() -> List[Path]:
    if not DOCS_DIR.exists():
        return []
    return sorted([p for p in DOCS_DIR.rglob("*") if p.is_file() and not any(
...
def _tool_definitions() -> List[Dict[str, Any]]:
    return [
        ...
        {
            "name": "list_docs",
            "description": "List Next.js doc file paths with pagination. Supports optional glob pattern like 01-app/**/*.mdx",
            "inputSchema": {
                "type": "object",
                "properties": {
        ...
        {
            "name": "search_docs",
            "description": "Search Next.js docs by path and content with pagination.",
            "inputSchema": {
                "type": "object",
                "properties": {
        ...
        {
            "name": "read_doc",
            "description": "Read one Next.js doc file by path with optional chunking via offset/length.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "path": {
                        "type": "string",
                        "description": "Repo-relative path, such as 01-app/02-guides/01-authentication.mdx",
                    },
                    "offset": {
                        "type": "integer",
                        "minimum": 0,
...
def _tool_get_stats(arguments: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for f in files:
        try:
            ...
            pass
    return {"content": [{"type": "text", "text": _json_dumps({
...
def _tool_search_docs(arguments: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for path in _all_doc_files():
        ...
        content_match = False
        snippet = ""
        if path.suffix.lower() in {".md", ".mdx", ".txt", ".json"}:
            try:
...
def _tool_read_doc(arguments: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not _is_relative_to(candidate, DOCS_DIR):
        raise ValueError("path must be inside docs directory")
...
def _handle_request(req: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if method == "initialize":
        return _ok_response(req_id, {
            ...
            "serverInfo": {"name": "nextjs-docs-mcp", "version": "1.0.0"},
        })
